class CurrencyInput {
    ...
    format(e) {
        ...
        if (typeof e != 'undefined') {
            input = this.getEventValue(e).replaceAll(sap.symbol, '');
    ...
    reInit(options) {
        ...
        if (this.getDecimalCharacter() === '/' || this.getSeparationCharacter() === '/') {
            throw new TypeError('The divider characters must not be a forward slash.');
        }

        this.disableCents = typeof options.disableCents === 'boolean' && options.disableCents;
        this.preventInputFromIME = typeof options.preventInputFromIME !== 'undefined' &&
            typeof options.preventInputFromIME === 'boolean' &&
            !options.preventInputFromIME;
    ...
    validateInput(e) {
        ...
        if (input !== '-') {
            if (input.slice(-1) === decimalChar) {
                checkValue = parseFloat(input.replace(new RegExp('\\'+decimalChar), '.') + Math.pow(10, -decimals).toString().replace('0.', ''));
            } else {
        ...
        this.getEventValue(e).includes('-'+this.getSeparationCharacter())) {
            ...
            console.log(typeof this.invalidCallback);
